...
def main():
    ...
    parser.add_argument('--output_logits', type=str, default='logits.npy', help='Path to save logits')
    ...
    model = AutoModelForTokenClassification.from_pretrained("roberta-large", num_labels=len(tag2id)).to(device)
    ...
    logits = evaluate_model(trained_model, test_loader, device)
    np.save(args.output_logits, logits)
    print(f"Logits saved to {args.output_logits}")
